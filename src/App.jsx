...
const DEMO_RESULT = {
  ...
  code_type: "recursive algorithm",
  overall_summary: "Yeh code ek number ka factorial nikalta hai recursion use karke — matlab function khud ko baar-baar call karta hai jab tak n, 1 tak nahi pahunch jaata.",
  how_it_works: "compute(n) function check karta hai ki n, 1 ya usse chhota hai kya — agar haan toh 1 return karta hai (base case). Warna n ko compute(n-1) ke result se multiply karke return karta hai, jisse ek chain ban jaati hai jab tak base case na aa jaaye.",
  lines: [
    { line_number: 1, code: "public class Factorial {", explanation: "Factorial naam ki ek class define ho rahi hai — Java mein har code class ke andar hona zaroori hai." },
    { line_number: 2, code: "    public static int compute(int n) {", explanation: "compute naam ka function ban raha hai jo ek number (n) leta hai aur ek number return karta hai." },
    { line_number: 3, code: "        if (n <= 1) {", explanation: "Check ho raha hai — agar n, 1 ya usse chhota hai." },
    { line_number: 4, code: "            return 1;", explanation: "Agar sach hai, toh 1 return kar do — yahi base case hai jo recursion ko rukwata hai." },
    { line_number: 5, code: "        }", explanation: "if block band ho raha hai." },
    { line_number: 6, code: "        return n * compute(n - 1);", explanation: "Warna, n ko (n-1) ke factorial se multiply karke return karo — yahi recursive call hai." },
    { line_number: 7, code: "    }", explanation: "compute function band ho raha hai." },
    { line_number: 9, code: "    public static void main(String[] args) {", explanation: "main function — program yahin se shuru hota hai." },
    { line_number: 10, code: "        int result = compute(5);", explanation: "compute(5) call karke result variable mein store kiya ja raha hai." },
    { line_number: 11, code: "        System.out.println(result);", explanation: "Result ko console pe print kiya ja raha hai." },
    { line_number: 12, code: "    }", explanation: "main function band ho raha hai." },
    { line_number: 13, code: "}", explanation: "Class band ho rahi hai." },
  ],
  keywords: [
    { term: "public", meaning: "Iska matlab hai yeh kahin se bhi access ho sakta hai, koi restriction nahi." },
    { term: "static", meaning: "Iska matlab function object banaye bina direct call ho sakta hai." },
    { term: "int", meaning: "Integer — poora number (bina decimal ke) store karne wala data type." },
    { term: "recursion", meaning: "Jab koi function khud ko andar hi andar call kare, jab tak ek stopping condition (base case) na aa jaaye." },
  ],
  dependencies: [],
  risks: [
    { issue: "Bade n (jaise n=10000) ke liye yeh stack overflow error de sakta hai kyunki recursion bahut zyada calls create karta hai.", severity: "medium", category: "logic-bug" },
    { issue: "Negative number diya (jaise -5) toh yeh kabhi rukega nahi, infinite recursion ho jayega.", severity: "high", category: "logic-bug" },
  ],
  errors: [],
  complexity: { level: "beginner", note: "Time complexity O(n) hai — n calls hoti hain. Space complexity bhi O(n) hai kyunki har call stack mein jama hoti hai." },
  example: { input: "compute(5)", output: "120", note: "5 × 4 × 3 × 2 × 1 = 120" },
  trace_walkthrough: [
    { step: 1, description: "compute(5) call hota hai", variables_state: "n = 5" },
    { step: 2, description: "n <= 1 false hai, toh compute(4) call hota hai", variables_state: "n = 5, waiting for compute(4)" },
    { step: 3, description: "Yeh chain compute(1) tak jaati hai jahan base case trigger hota hai", variables_state: "n = 1, returns 1" },
    { step: 4, description: "Ab saari calls wapas multiply hoke unwind hoti hain: 1×2×3×4×5", variables_state: "final result = 120" },
  ],
  call_graph: [
    { caller: "main", callee: "compute" },
    { caller: "compute", callee: "compute (khud ko)" },
  ],
  data_flow: [
    { variable: "n", lifecycle: "declared as parameter -> check hota hai line 3 pe -> use hota hai line 6 mein" },
    { variable: "result", lifecycle: "declared line 10 -> compute() ka return value store karta hai -> print hota hai line 11" },
  ],
  fixes: [
    { issue: "Negative input handle nahi hota", suggested_fix: "Function ke start mein check add karo: if (n < 0) throw new IllegalArgumentException()" },
  ],
  scores: { security: 95, quality: 80, performance: 65 },
  tips: [
    "Bade numbers ke liye iterative (loop-based) version zyada safe hota hai stack overflow se bachne ke liye.",
    "Recursion samajhne ka best tarika hai — chhote example (jaise n=3) ko haath se trace karna.",
  ],
};
  ...
  const handleExplain = useCallback(async () => {
    ...
    if (DEMO_MODE) {
      if (code.trim() === SAMPLE_CODE.trim()) {
        ...
        setError(
          outputLang === "hindi"
            ? "डेमो मोड चालू है (API billing सेट नहीं है) — कृपया ऊपर 'नमूना कोड आज़माएं' बटन दबाएं ताकि एक तैयार उदाहरण देख सकें।"
            : outputLang === "english"
            ? "Demo mode is on (API billing isn't set up) — please click the 'Try sample code' button above to see a ready example."
            : "Demo mode chalu hai (API billing set nahi hai) — upar 'Sample code try karo' button dabao taaki ek ready example dekh sako."
        );
        return;
      }
    }
  ...
  return (
    <div className={`cg-root ${outputLang === "hindi" ? "lang-hindi" : ""}`}>
      ...
      <div className="cg-header">
        {DEMO_MODE && (
          <div style={{ display: "inline-block", background: "rgba(232,163,61,0.15)", border: "1px solid var(--marigold)", color: "#F4C878", fontSize: 12, padding: "4px 12px", borderRadius: 999, marginBottom: 12, fontFamily: "var(--font-body)", fontWeight: 600 }}>
            ⚡ Demo Mode — sample code pe pre-generated result, live API abhi off hai
          </div>
        )}
